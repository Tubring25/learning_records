...
<script>
import { getExpressList, getTakeOutList } from '@/api/server'
export default {
  data() {
    return {
      activeName: 'first',
  ...
  methods: {
    getList() {
      getExpressList({Server_ID: this.userInfo.id}).then(res=>{
        if(res.code) {
          this.expressList = res.data
        }
      })
      getTakeOutList({Server_ID: this.userInfo.id}).then(res=>{
        if(res.code) {
